	...
	if (!("doctype".equals(sTagName)|| "?xml".equals(sTagName)))
		...
		public Reader handleEntity(Reader toParse, final StringBuffer stringBuffer) throws Exception{
			...
			while (iChar != -1){
				cChar = iChar;
				result.append(cChar);
				stringBuffer = stringBufferMethodA((char) cChar,stringBuffer,  result);
		...
		public StringBuffer stringBufferMethodA(char cChar,StringBuffer stringBuffer1, StringBuffer result) {
			if (cChar == ';'){
